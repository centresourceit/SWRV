...
/**
 * Loader function that retrieves the category data from the server.
 * @param {LoaderArgs} props - The loader arguments.
 * @returns {Promise<LoaderResult>} A promise that resolves to the category data.
 */
export const loader: LoaderFunction = async (props: LoaderArgs) => {
    const cateogry = await axios({
        method: 'post',
        url: `${BaseUrl}/api/getcategory`,
    });
    return json({ cateogry: cateogry.data.data });
}



/**
 * A functional component that represents a Category.
 * @returns JSX elements representing the Category component.
 */
const Category = () => {
    const cateogry = useLoaderData().cateogry;
    const pagination = usePagination(cateogry);
    ...
    /**
     * Renders a component that displays a modal with information about a category, including its ID, name, and code.
     * Allows the user to close the modal, delete the category, or edit the category's name and code.
     * Also includes a section to add a new category.
    ...
    return (
        <>
            ...
            <div className="grow bg-[#1b2028] my-2 rounded-md p-4 w-full">
                <div className="flex">
                    <h1 className="text-white font-medium text-xl">Category</h1>
                        ...
                        pagination.paginatedItems.map((val: any, index: number) => {
                            return (
                                <div key={index} className="bg-[#31353f] hover:bg-slate-800 rounded-md flex px-4 py-2 my-2 text-white font-medium text-md flex-nowrap">
                                    ...
                                    <div className="grow"></div>
                                    <div className="w-40">{val.categoryName}</div>
                                    <div className="grow"></div>
                                    <div className="w-40">{val.categoryCode}</div>
                                    <div className="grow"></div>
