...
const EVENT = () => {
  ...
  /**
   * Initializes a state variable with an object containing various properties.
   * @param {any} editData - The initial values for the state object.
   * @returns A state variable and a function to update its value.
   */
  ...
  /**
   * Edit a specific item identified by its ID.
  ...
  /**
   * Deletes a button with the specified ID from the server.
   ...
   * Validates the input fields and displays an error message if any field is empty or not selected.
   * If all fields are filled, it uploads the image file, constructs the data object, and sends a POST request to the server.
  ...
  /**
   * Performs a search operation based on the provided search keyword.
   * @returns None
   */
  const search = async () => {
    ...
    const data = await axios.post(`${BaseUrl}/api/search-neb`, { search: searchRef.current?.value, type: 3 });
      ...
      <div className="grow bg-[#1b2028] my-2 rounded-md p-4 w-full">
        <div className="flex">
          <div className="flex items-center gap-2 w-full">
            <h1 className="text-white font-medium text-xl">EVENT</h1>
