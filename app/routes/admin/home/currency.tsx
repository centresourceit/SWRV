...
/**
 * Loader function that retrieves the currency data from the server.
 * @param {LoaderArgs} props - The loader arguments.
 * @returns {Promise<{ currency: any }>} - A promise that resolves to an object containing the currency data.
 */
export const loader: LoaderFunction = async (props: LoaderArgs) => {
    ...
    return json({ currency: currency.data.data });
}



const Currency = () => {
    const currency = useLoaderData().currency;
    const pagination = usePagination(currency);
    ...
    return (
        <>
            ...
            <div className="grow bg-[#1b2028] my-2 rounded-md p-4 w-full">
                <div className="flex">
                    <h1 className="text-white font-medium text-xl">Currency</h1>
                        ...
                        pagination.paginatedItems.map((val: any, index: number) => {
                            return (
                                <div key={index} className="bg-[#31353f] hover:bg-slate-800 rounded-md flex px-4 py-2 my-2 text-white font-medium text-md flex-nowrap">
                                    ...
                                    <div className="w-20"></div>
                                    <div className="w-44">{val.currencyName}</div>
                                    <div className="grow"></div>
                                    <div className="w-44">{he.decode(val.currencyAsciiSymbol)}</div>
                                    {/* <div className="grow"></div>
