...
/**
 * Loader function that retrieves the campaign type from the server.
 * @param {LoaderArgs} props - The loader arguments.
 * @returns {Promise<{ campaignType: string }>} - A promise that resolves to an object containing the campaign type.
 */
export const loader: LoaderFunction = async (props: LoaderArgs) => {
    const campaignType = await axios({
        method: 'post',
        url: `${BaseUrl}/api/get-campaign-type`,
    });
    return json({ campaignType: campaignType.data.data });
}



/**
 * Represents a component that displays and manages campaign types.
 * @returns None
 */
const CampaignType = () => {
    const campaignType = useLoaderData().campaignType;
    const pagination = usePagination(campaignType);
    ...
    /**
     * Renders a component that displays a modal with information about a selected item, a confirmation dialog for deleting an item, and a form for editing an item.
                        ...
                        pagination.paginatedItems.map((val: any, index: number) => {
                            return (
                                <div key={index} className="bg-[#31353f] hover:bg-slate-800 rounded-md flex px-4 py-2 my-2 text-white font-medium text-md flex-nowrap">
                                    ...
                                    <div className="w-20"></div>
                                    <div className="w-44">{val.categoryName}</div>
                                    <div className="grow"></div>
                                    <div className="w-44 text-center">
                                        <img className="w-8 h-8 object-cover object-center rounded-md" src={val.categoryPicUrl} alt="logo" />
                                    </div>
                                    {/* <div className="grow"></div>
